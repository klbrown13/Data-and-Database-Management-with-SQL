...
SELECT sale_year, COUNT(*) AS total_female_buyers
FROM customer_spending
...
SELECT age, sub_category, CAST(AVG(quantity) AS INTEGER) AS avg_quantity,
...
SELECT sale_year, category, SUM(revenue) AS total_revenue, SUM(cost) AS total_cost, 
...
SELECT gender, category, MAX(unit_cost) AS high_cost, 
...
SELECT category, country, MIN(age) AS youngest_customer, MAX(age) AS oldest_customer,
